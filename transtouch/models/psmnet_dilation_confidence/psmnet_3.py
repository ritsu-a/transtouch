...
class hourglass2D(nn.Module):
    def __init__(self, inplanes, dilation):
        ...
        self.conv1 = nn.Sequential(
            nn.Conv2d(inplanes, inplanes * 2, kernel_size=3, stride=2, padding=dilation, dilation=dilation),
            nn.BatchNorm2d(inplanes * 2),
            nn.ReLU(inplace=True),
        )

        self.conv2 = nn.Sequential(
            nn.Conv2d(inplanes * 2, inplanes * 2, kernel_size=3, stride=1, padding=dilation, dilation=dilation),
            nn.BatchNorm2d(inplanes * 2),
        )

        self.conv3 = nn.Sequential(
            nn.Conv2d(inplanes * 2, inplanes * 2, kernel_size=3, stride=2, padding=dilation, dilation=dilation),
            nn.BatchNorm2d(inplanes * 2),
            nn.ReLU(inplace=True),
        )
        ...
        self.conv4 = nn.Sequential(
            nn.Conv2d(inplanes * 2, inplanes * 2, kernel_size=3, stride=1, padding=dilation, dilation=dilation),
            nn.BatchNorm2d(inplanes * 2, inplanes * 2),
            nn.ReLU(inplace=True),
        )
...
class PSMNetDilation(nn.Module):
    def __init__(
        ...
        self.domain_conv0 = nn.Sequential(
        ...
        self.domain_conv1 = nn.Sequential(
            convbn(32, 32, 3, 1, 1, 1),
            nn.ReLU(inplace = True),
            convbn(32, 32, 3, 1, 1, 1),
        )

        self.domain_hourglass = hourglass2D(32, dilation)

        self.domain_classif = nn.Sequential(
            convbn(32, 32, 3, 1, 1, 1),
            nn.ReLU(inplace=True),
    ...
    def forward(self, data_batch):
        ...
        if self.training:
            pred_dict = {
                ...
                "pred3": pred3,
                "conf_map": conf_map,
                "domain_confidence": domain_out,
                "domain_confidence1": domain_out1,
            }
        else:
            pred_dict = {
                "pred1": pred1,
        ...
        # TODO: delete the uninterpolated domain out 1
