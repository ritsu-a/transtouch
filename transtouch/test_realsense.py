...
def parse_args():
    parser = argparse.ArgumentParser(description="Evaluate realsense performance")
...
def main():
    ...
    logger = setup_logger(f"ActiveZero2.test RealSense", output_dir, rank=0, filename=f"log.realsense.{run_name}.txt")
    logger.info(args)
    ...
    file_dir = osp.join(output_dir, "realsense_test")
    eval_tic = time.time()
