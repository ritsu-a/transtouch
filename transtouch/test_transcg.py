...
def parse_args():
    parser = argparse.ArgumentParser(description="Evaluate transcg performance")
...
def register_depth(view_folder, data_dir):
    ...
    # if (view_folder / "1024_Transcg_real.png").exists():
    if False: 
        ...
        rgb_cam_depth = np.load(f"/share/pengyang/TransCG/outputs_0228/{data_dir}/depth.npy") / 1000.0
        ...
        cv2.imwrite(view_folder / "1024_Transcg_real.png", depth_u16)
        vis_depth = visualize_depth(depth_u16)
        cv2.imwrite(view_folder / "1024_Transcg_real_colored.png", vis_depth)
...
def main():
    ...
    logger = setup_logger(f"ActiveZero2.test Transcg",output_dir, rank=0, filename=f"log.transcg.{run_name}.txt")
    logger.info(args)
    ...
    file_dir = osp.join(output_dir, "transcg_test")
    eval_tic = time.time()
    ...
    logger.info("Transcg Test total_time: {:.2f}s".format(epoch_time_eval))
    logger.info("Transcg Eval Metric: \n" + metric.summary())
